...
export default function FriendsPage() {
  ...
  async function loadData() {
    ...
    const { data: f1 } = await supabase
      .from("friendships")
      .select("*, profile:profiles!friendships_addressee_id_fkey(id,username,avatar_url,display_id)")
      .eq("requester_id", user.id)
      .eq("status", "accepted");
    const { data: f2 } = await supabase
      .from("friendships")
      .select("*, profile:profiles!friendships_requester_id_fkey(id,username,avatar_url,display_id)")
      .eq("addressee_id", user.id)
      .eq("status", "accepted");
    setFriends(([...(f1 || []), ...(f2 || [])]) as any);
      ...
      <div className="bg-[#1a1a30] border border-[#2a2a44] rounded-xl p-4 mb-6">
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder={searchMode === "name" ? "搜索用户名..." : "输入 UID 号码..."}
          />
              ...
              <div key={r.id} className="flex items-center justify-between bg-[#12122a] rounded-lg px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {r.display_id && <span className="text-[10px] text-[#22d3ee] ml-2 font-mono">UID:{r.display_id}</span>}
                  </div>
                </div>
          ...
          {visibleRequests.map((r) => (
            <div key={r.id} className="flex items-center justify-between bg-[#1a1a30] border border-[#2a2a44] rounded-xl px-4 py-3">
              ...
              <div className="flex gap-1.5">
                <button onClick={() => acceptRequest(r.id)} className="px-3 py-1.5 rounded-lg bg-[#22c55e]/20 text-[#22c55e] text-xs hover:bg-[#22c55e]/30 transition-colors flex items-center gap-1" title="同意">
                  <UserCheck size={13} /> 同意
                </button>
                <button onClick={() => rejectRequest(r.id)} className="px-3 py-1.5 rounded-lg bg-[#f43f5e]/20 text-[#f43f5e] text-xs hover:bg-[#f43f5e]/30 transition-colors flex items-center gap-1" title="拒绝">
                  <UserX size={13} /> 拒绝
                </button>
                <button onClick={() => ignoreRequest(r.id)} className="px-3 py-1.5 rounded-lg border border-[#3a3a50] text-[#9090a8] text-xs hover:text-white hover:border-[#6366f1] transition-colors flex items-center gap-1" title="忽略">
                  <EyeOff size={13} /> 忽略
                </button>
              </div>
            </div>
